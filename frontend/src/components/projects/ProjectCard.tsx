...
const ProjectCard = ({ project, index, onViewDetails, onEdit, onDelete, showAdminControls, adminUsers = [] }: ProjectCardProps) => {
  ...
  const displayedContributors = contributorNames.slice(0, 2)
  const remainingCount = Math.max(0, contributorNames.length - 2)
  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        {/* Title */}
        <h3 className="text-xl font-bold mb-3 text-gray-800 hover:text-primary-600 transition-colors line-clamp-2 h-14">
          {project.title}
        </h3>

        {/* Description with LinkedIn Icon */}
        <div className="mb-4">
          <p className="text-gray-600 text-sm leading-relaxed">
            ...
            <button
              onClick={(e) => {
                e.stopPropagation()
                handleLinkClick(e, project.linkedInUrl, 'LinkedIn link')
              }}
              className="inline-flex items-center justify-center w-5 h-5 bg-blue-700 hover:bg-blue-800 text-white rounded transition-colors align-middle"
